...
draw_text(x,y-30,"Recruit Hero: Goku");
draw_set_font(fnt_RPS);



draw_text(x,y+30,"Cost: 25M");
}
else{
	var type = "Cost: ";
...
if(global.GokuLVL=25||global.GokuLVL=50||global.GokuLVL=100||global.GokuLVL=150){
   ...
   var rep_string = string(round(global.Goku_cost*multiplierUpgrade));
var pow=1;
var Multi="K";
...
if(!show_stats){
draw_set_font(fnt_Reputation);
   draw_text(x+75,y-35,"Goku   LVL    "+string(global.GokuLVL));

if(string_length(rep_string)<4){
	draw_set_font(fnt_RPS);
draw_text(x+75,y,type+string(global.Goku_cost*multiplierUpgrade));
}
else {
	draw_set_font(fnt_RPS);
draw_text(x+75,y,type+string_format((global.Goku_cost*multiplierUpgrade)/power(1000,pow),3,3)+Multi);
}
}
	else{
		draw_set_halign(fa_left);
	draw_set_font(fnt_RPS)
	draw_text(x-20,y-20,"Goku   LVL    "+string(global.GokuLVL));
	if(string_length(rep_string)<4){
draw_text(x-20,y,type+string(global.Goku_cost*multiplierUpgrade));	
}
else {	
draw_text(x-20,y,type+string_format((global.Goku_cost*multiplierUpgrade)/power(1000,pow),3,3)+Multi);
}
...
var rps_string = ((global.GokuLVL*8000)*power(2,global.Goku_multiplier))*frenzy_multi*(1+(global.SovereignLVL*25)/100);
RPC= ((global.GokuLVL*8000)*power(2,global.Goku_multiplier))*frenzy_multi*(1+(global.SovereignLVL*25)/100);
