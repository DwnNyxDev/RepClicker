...
draw_text(x,y-30,"Recruit Hero: Leon");
draw_set_font(fnt_RPS);
draw_text(x,y+30,"Cost: 2.555M");
}
else{
	var type = "Cost: ";
...
if(global.LeonLVL=25||global.LeonLVL=50||global.LeonLVL=100||global.LeonLVL=150){
   ...
   var rep_string = string(round(global.Leon_cost*multiplierUpgrade));
var pow=1;
var Multi="K";
...
if(!show_stats){
draw_set_font(fnt_Reputation);
   draw_text(x+75,y-35,"Leon   LVL    "+string(global.LeonLVL));

if(string_length(rep_string)<4){
	draw_set_font(fnt_RPS);
draw_text(x+75,y,type+string(global.Leon_cost*multiplierUpgrade));
}
else {
	draw_set_font(fnt_RPS);
draw_text(x+75,y,type+string_format((global.Leon_cost*multiplierUpgrade)/power(1000,pow),3,3)+Multi);
}
}
else{
	draw_set_halign(fa_left);
	draw_set_font(fnt_RPS)
	draw_text(x-20,y-20,"Leon   LVL    "+string(global.LeonLVL));
	if(string_length(rep_string)<4){
draw_text(x-20,y,type+string(global.Leon_cost*multiplierUpgrade));	
}
else {	
draw_text(x-20,y,type+string_format((global.Leon_cost*multiplierUpgrade)/power(1000,pow),3,3)+Multi);
}
...
var rps_string = ((global.LeonLVL*1500)*power(2,global.Leon_multiplier))*frenzy_multi*(1+(global.SovereignLVL*25)/100);
RPC= ((global.LeonLVL*1500)*power(2,global.Leon_multiplier))*frenzy_multi*(1+(global.SovereignLVL*25)/100);
