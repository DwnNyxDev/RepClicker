...
draw_text(x,y-30,"Recruit Hero: Luffy");
draw_set_font(fnt_RPS);



draw_text(x,y+30,"Cost: 333M");
}
else{
	var type = "Cost: ";
...
if(global.LuffyLVL=25||global.LuffyLVL=50||global.LuffyLVL=100||global.LuffyLVL=150){
   ...
   var rep_string = string(round(global.Luffy_cost*multiplierUpgrade));
var pow=1;
var Multi="K";
...
if(!show_stats){
draw_set_font(fnt_Reputation);
   draw_text(x+75,y-35,"Luffy   LVL    "+string(global.LuffyLVL));

if(string_length(rep_string)<4){
	draw_set_font(fnt_RPS);
draw_text(x+75,y,type+string(global.Luffy_cost*multiplierUpgrade));
}
else {
	draw_set_font(fnt_RPS);
draw_text(x+75,y,type+string_format((global.Luffy_cost*multiplierUpgrade)/power(1000,pow),3,3)+Multi);
}
}
else{
	draw_set_halign(fa_left);
	draw_set_font(fnt_RPS)
	draw_text(x-20,y-20,"Luffy   LVL    "+string(global.LuffyLVL));
	if(string_length(rep_string)<4){
draw_text(x-20,y,type+string(global.Luffy_cost*multiplierUpgrade));	
}
else {	
draw_text(x-20,y,type+string_format((global.Luffy_cost*multiplierUpgrade)/power(1000,pow),3,3)+Multi);
}
...
var rps_string = ((global.LuffyLVL*48000)*power(2,global.Luffy_multiplier))*frenzy_multi*(1+(global.SovereignLVL*25)/100);
RPC= ((global.LuffyLVL*48000)*power(2,global.Luffy_multiplier))*frenzy_multi*(1+(global.SovereignLVL*25)/100);
