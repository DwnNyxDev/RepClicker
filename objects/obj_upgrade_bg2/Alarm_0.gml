...
repeat(global.particleIntensity){
	...
	part.depth=-5;
	if(global.Verde_multiplier>=2){
	part.shape=choose("circle","square","diamond");
	}
	else if(global.Verde_multiplier>=1){
	part.shape=choose("circle","square");
	}
	else if(global.Verde_multiplier=0){
		part.shape="circle";
	}
	part.color=choose(c_green,make_color_rgb(62,48,26),make_color_rgb(182,161,104));
