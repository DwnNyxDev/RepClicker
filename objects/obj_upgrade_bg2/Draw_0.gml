/// @descrQadziption Insert description here
...
draw_text(x,y-30,"Recruit Hero: Verde");
draw_set_font(fnt_RPS);
draw_text(x,y+30,"Cost: 150");
}
else{
...
var rep_string = string(round(global.Verde_cost*multiplierUpgrade));
var pow=1;
var Multi="K";
...
if(!show_stats){
	draw_set_font(fnt_Reputation);
   draw_text(x+75,y-35,"Verde   LVL    "+string(global.VerdeLVL));
   if(string_length(rep_string)<4){
	draw_set_font(fnt_RPS);
draw_text(x+75,y,type+string(global.Verde_cost*multiplierUpgrade));
}
else {
	draw_set_font(fnt_RPS);
draw_text(x+75,y,type+string_format((global.Verde_cost*multiplierUpgrade)/power(1000,pow),3,3)+Multi);
}
	...
	draw_set_font(fnt_RPS)
	draw_text(x-20,y-20,"Verde   LVL    "+string(global.VerdeLVL));
	if(string_length(rep_string)<4){
draw_text(x-20,y,type+string(global.Verde_cost*multiplierUpgrade));	
}
else {	
draw_text(x-20,y,type+string_format((global.Verde_cost*multiplierUpgrade)/power(1000,pow),3,3)+Multi);
}
...
var rps_string = ((global.VerdeLVL*1)*power(2,global.Verde_multiplier))*frenzy_multi*(1+(global.SovereignLVL*25)/100);
RPC= ((global.VerdeLVL*1)*power(2,global.Verde_multiplier))*frenzy_multi*(1+(global.SovereignLVL*25)/100);
